...
if [[ ${INSTALL_WESELY_EXO_COLDENS} -eq 1 ]]; then

  if [[ ! -e ${LAUNCH_DIR}/wes-coldens.tar ]]; then
    echo "wes-coldesn.tar does not exist in ${LAUNCH_DIR}"
    exit 1
  fi

  if [[ ! -d ${WESCOL_DIR} ]]; then
    echo "Directory (${WESCOL_DIR}) does not exist"
    echo "Creating the directory ....."
    mkdir -p ${WESCOL_DIR}
  fi

  cd ${WESCOL_DIR}
  cp ${LAUNCH_DIR}/wes-coldens.tar .
  tar -xvf wes-coldens.tar
  ...
  if [[ ! -f ./exo_coldens ]]; then
    echo "ERROR: exo_coldens has not been created"
    exit 1
  fi
  if [[ ! -f ./wesely ]]; then
    echo "ERROR: wesely has not been created"
    exit 1
  fi

  rm ${WESCOL_DIR}/wes-coldens.tar 
...
if [[ ${INSTALL_MEGAN} -eq 1 ]]; then

  if [[ ! -e ${LAUNCH_DIR}/MEGAN.tar ]]; then
    echo "MEGAN.tar does not exist in ${LAUNCH_DIR}"
    exit 1
  fi

  if [[ ! -d ${MEGAN_DIR} ]]; then
    echo "Directory (${MEGAN_DIR}) does not exist"
    echo "Creating the directory ....."
    mkdir -p ${MEGAN_DIR}
  fi

  cd ${MEGAN_DIR}
  cp ${LAUNCH_DIR}/MEGAN.tar .
  tar -xvf MEGAN.tar 

  cd ${MEGAN_DIR}/MEGAN
  sed -i "s|<module_intel>|${module_intel}|" ./make_util
  sed -i "s|<module_openmpi>|${module_openmpi}|" ./make_util
  sed -i "s|<module_netcdfc>|${module_netcdfc}|" ./make_util
  sed -i "s|<module_netcdff>|${module_netcdff}|" ./make_util
  sed -i "s|<module_hdf5>|${module_hdf5}|" ./make_util
  sed -i "s|<module_jasper>|${module_jasper}|" ./make_util

  ./make_util megan_bio_emiss

  if [[ ! -f ./megan_bio_emiss ]]; then
    echo "ERROR: megan_bio_emiss has not been created"
    exit 1
  fi

  rm ${MEGAN_DIR}/MEGAN.tar
...
if [[ ${INSTALL_MOZBC} -eq 1 ]]; then

  if [[ ! -e ${LAUNCH_DIR}/MOZBC.tar ]]; then
    echo "wes-coldesn.tar does not exist in ${LAUNCH_DIR}"
    exit 1
  fi

  if [[ ! -d ${MOZBC_DIR} ]]; then
    echo "Directory (${MOZBC_DIR}) does not exist"
    echo "Creating the directory ....."
    mkdir -p ${MOZBC_DIR}
  fi

  cd ${MOZBC_DIR}
  cp ${LAUNCH_DIR}/MOZBC.tar .
  tar -xvf MOZBC.tar

  cd ${MOZBC_DIR}/MOZBC
  sed -i "s|<module_intel>|${module_intel}|" ./make_mozbc
  sed -i "s|<module_openmpi>|${module_openmpi}|" ./make_mozbc
  sed -i "s|<module_netcdfc>|${module_netcdfc}|" ./make_mozbc
  sed -i "s|<module_netcdff>|${module_netcdff}|" ./make_mozbc
  sed -i "s|<module_hdf5>|${module_hdf5}|" ./make_mozbc
  sed -i "s|<module_jasper>|${module_jasper}|" ./make_mozbc

  ./make_mozbc

  if [[ ! -e ${MOZBC_DIR}/MOZBC/mozbc ]]; then
    echo "ERROR: mozbc has not been created"
    exit 1
  fi
  
  rm ${MOZBC_DIR}/MOZBC.tar
...
MEGAN_DIR="/proju/wrf-chem/onishi/MEGAN/WRFrun_TEST/"
MEGAN_exe="/proju/wrf-chem/onishi/MEGAN/NEW_MEGAN_SPIRIT/megan_bio_emiss"
MEGAN_data_DIR="/proju/wrf-chem/onishi/MEGAN/DATA_TOTAL"

WESCOL_DIR="/proju/wrf-chem/onishi/wes-coldens-spirit/"
wesely_exe="/proju/wrf-chem/onishi/wes-coldens-spirit/wesely"
exo_coldens_exe="/proju/wrf-chem/onishi/wes-coldens-spirit/exo_coldens"

MOZBC_DIR="/proju/wrf-chem/onishi/MOZART/MOZBC_ifort_TEST/"
mozbc_exe="/proju/wrf-chem/onishi/MOZART/MOZBC_ifort_TEST/mozbc"
mozbc_data_DIR="/proju/wrf-chem/onishi/MOZART/"
mozbc_data="cesm-ALPACA-0001.nc"

wrfchemi_python="./create_wrfchemi.py3"
wrfchemi_DIR="./WRFrun_TEST/"
...
if tail -n 1 ./PythonEmission${job_id}.out | grep -q "Done"; then
  ...
  while [ $t -le $end_sec ]; do
    ...
    filename="wrfchemi_d01_${file_datetime}"
...
#SBATCH --partition=zen4                      # Nom d'une partition pour une ex̩tion cpu
#SBATCH --ntasks=64                         # nombre de taches
#SBATCH --ntasks-per-node=64               # nombre de taches MPI par noeud
